...
for index, row in df.iterrows():
    if "Company" in row[col_company]:
        ...
        "JUUL" in row[col_company]
        or "Juul" in row[col_company]
        or "JUUl" in row[col_company]
    ):
    ...
    if row[col_effective_date] not in final_dict:
        ...
        for i in range(len(final_dict[row[col_effective_date]])):
            if final_dict[row[col_effective_date]][i][0] == company_name:
                no_of_employees = int(final_dict[row[col_effective_date]][i][1]) + int(
                    row[col_no_of_employees]
                )
                final_dict[row[col_effective_date]][i] = (company_name, no_of_employees)
                counter = True
                break
        if not counter:
            final_dict[row[col_effective_date]].append(
                (company_name, row[col_no_of_employees])
            )
...
plt.xticks(x_values, rotation="vertical", fontsize=6)
for i, txt in enumerate(x_labels):
    ax.annotate(
        txt, (x_values[i], y_values[i]), va="bottom", rotation=45, c=color_list[i]
    )
...
plt.savefig(
    os.path.join(os.getcwd(), f'graph_{time.strftime("%Y-%m-%d_%H:%M:%S")}.png'),
    dpi=300,
    bbox_inches="tight",
)
