...
def _push_origin_main(token: str) -> bool:
    ...
    The remote is verified with urlparse (scheme AND hostname), not a
    substring check — ``"github.com" in url`` would happily inject the token
    into ``https://evil.com/github.com``.
    """
    url_modified = False
    original_url = ""
    if token:
